...
CREATE TABLE offersproducts (
	...
	FOREIGN KEY (package) REFERENCES SERVICEPACKAGE(id) ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY (product) REFERENCES OPTIONALPRODUCT(name) ON DELETE CASCADE ON UPDATE CASCADE
	);
...
FOR EACH ROW
	UPDATE CUSTOMERORDER AS c SET
		...
		(SELECT O.monthlyfee FROM OPTIONALPRODUCT AS O WHERE O.name = new.product))
